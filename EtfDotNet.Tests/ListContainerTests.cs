...
public class ListContainerTests
{
    ...
    {
        var atom = EtfDecoder.DecodeType(EtfMemory.FromArray(new byte[]{(byte) EtfConstants.ListExt, 0, 0, 0, 0, (byte) EtfConstants.NilExt}));
    ...
    {
        var atom = EtfDecoder.DecodeType(EtfMemory.FromArray(new byte[]{(byte) EtfConstants.ListExt, 0, 0, 0, 1, (byte) EtfConstants.NilExt, (byte) EtfConstants.NilExt}));
        Assert.Equal(EtfConstants.ListExt, atom.Type);
        var list = atom.AsList();
        Assert.Single(list);
